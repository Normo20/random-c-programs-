...
namespace CompoundInterestCalculator
{
    public class CompoundCalculator
    {
        ...
        public static void OutputResult(double finalAmount)
        {
            ...
            Console.WriteLine($"The final ammount of money plus interest is: ${finalAmount:0.00}");
        } // end OutputResult
        ...
        public static double CalculateInterest(int menuOption)
        {
            ...
            if (menuOption == CALCULATE_DAILY || menuOption == CALCULATE_QUARTERLY)
            {
                if (menuOption == CALCULATE_DAILY)
                {
                    ...
                    finalAmount= CalculateCompundInterest(principal,interestRate,numYears);
                ...
                else
                {
                   ...
                   finalAmount= CalculateCompundInterest(principal, interestRate, numYears, numTimesCompounded);
        ...
        public static double CalculateCompundInterest (double principal, double interestRate, int numYears){


            double interestRate1 =interestRate;
            ...
            // (1 + r/n)
            double body = 1 + (interestRate1 / timesPerYear);

            // nt
            double exponent = timesPerYear * years;

            // P(1 + r/n)^nt
            finalAmount = principal1 * Math.Pow(body, exponent);
        ...
        public static double CalculateCompundInterest(double principal, double interestRate, int numYears,int numTimesCompounded)
        {
            double interestRate1 = interestRate;
            double timesPerYear = numTimesCompounded;
            double years = numYears;
            double finalAmount;
            double principal1 = principal;
